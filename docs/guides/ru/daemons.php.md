...
> Note: В параметр вписывается наименование класса с пространством имён, полностью (например так `\app\components\PdfProcessing`).
...
> Note: Идентичность файла проверяется путём получения MD5 хэша из строки: `object_id` + (MD5 хэш из строки: ссылка на файл + объём файла в байтах).
...
> Note: Данная настройка добавлена для ситуаций, когда при получении данных на обработку по API не указан параметр `callback`.
...
> Note: Устанавливается индивидуально для машины, где работает демон и целевая машина, на которую будут отправляться результаты.
[php.ini post-max-size](http://php.net/manual/ini.core.php#ini.post-max-size)
...
> Note: По умолчанию расширение файла берётся из результатов запроса по ссылке на источник.
...
> Note: При отсутствии указанных директорий, они создаются автоматически.
...
> Note: Полный путь к файлу формируется по формуле `target` + `web` + `date('/Y/m/d/H/i/')` + имя файла
